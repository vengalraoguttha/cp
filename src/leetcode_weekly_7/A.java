...
public class A {
    ...
    public boolean validWordAbbreviation(String word, String abbr) {
        ...
        for(int i = 0; i < abbr.length(); i++){
            if(abbr.charAt(i) >='a' && abbr.charAt(i) <= 'z'){
                ...
                value = 0;
                if(abbr.charAt(i) != word.charAt(current)){
                    return false;
                }
                current++;
            } else {
                value *= 10;
                value += abbr.charAt(i) - '0';
            }
        }
        if(current + value != word.length()) return false;
        return true;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        // code goes here
        A a = new A();
        System.out.println(a.validWordAbbreviation(fr.next(), fr.next()));
