...
public class A {
    ...
    private static boolean calculate(String s, String t, int i, int j){
        if(i == s.length()) return true;
        if(j == t.length()) return false;
        if(dp[i][j] != null) return dp[i][j];
        boolean res = false;
        if(s.charAt(i) == t.charAt(j)) {
            res = calculate(s, t, i, j + 1) || calculate(s, t, i + 1, j + 1);
        } else {
            res = calculate(s, t, i, j + 1);
        }
        dp[i][j] = res;
        return res;
    }
